...
while not rospy.is_shutdown():
	...
	distance=cube[:,1]
	angle=cube[:,0]
	#np.savetxt("HeightData.csv", cube, delimiter=",")
	cube=cube[np.unique(cube[:,[0]],return_index=True,axis=0)[1]]
	#np.savetxt("HeightData1.csv", cube, delimiter=",")
	...
	print(Def_Camera.shape)
	SensorData = np.concatenate((Def_Lidar_Data, Sonar_Data, Def_Camera), axis = 1)
	#if not os.path.exists('/home/andres/.ros/FullData.csv'): np.savetxt("FullData.csv", SensorData, delimiter=",")
	#np.savetxt("FullData.csv", SensorData, delimiter=",")

	#if os.path.exists('/home/andres/.ros/FullData.csv'):
	#	print("file found")
	...
	print("Execution time for all data:",time.process_time()- start)
	quit()
