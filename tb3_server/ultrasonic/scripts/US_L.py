...
def talker():
    distance_publisher = rospy.Publisher('/L_sonar_dist', Range, queue_size = 1)
    rospy.init_node('L_sonar', anonymous=True)
    min_range = 0.02
    max_range = 3.5
    while not rospy.is_shutdown():
        data = Range()
        data.header.stamp = rospy.Time.now()
        data.header.frame_id = "/sonarL_link"
        ...
        for i in range (20):
            ...
            C_sound_vel=(C_temp*34300)/20
            distance_S1 = pulse_duration_S1*(C_sound_vel/2)
            distance_S1 = round(distance_S1, 3)
            Med=distance_S1
        ...
        if (x_hat_k - Med) < 0.05 and (x_hat_k - Med) > -0.05: 
            ...
            print ("left:", x_hat_k, "left unfiltered:", Med)
            print ("____________")
        else:
            print ("Kalman Filter for Left Sensor is Adjusting")
        rospy.sleep(1)
