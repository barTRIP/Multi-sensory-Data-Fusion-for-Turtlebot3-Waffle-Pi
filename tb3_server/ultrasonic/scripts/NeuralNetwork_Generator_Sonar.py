...
# load dataset
dataframe = read_csv("FullData_SonarFusion.csv", delim_whitespace=False, header=None)
...
model = Sequential()
model.add(Dense(2, input_dim=2, kernel_initializer="he_uniform", bias_initializer='zeros',activation='relu'))
model.add(Dense(60, kernel_initializer="he_uniform", bias_initializer='zeros',activation='relu'))
model.add(Dense(60, kernel_initializer="he_uniform", bias_initializer='zeros',activation='relu'))
model.add(Dense(60, kernel_initializer="he_uniform", bias_initializer='zeros',activation='relu'))
model.add(Dense(60, kernel_initializer="he_uniform", bias_initializer='zeros',activation='relu'))
model.add(Dense(60, kernel_initializer="he_uniform", bias_initializer='zeros',activation='relu'))
...
#Compiles model
model.compile(Adam(lr=0.0015, decay=2/Epochs), loss="mean_squared_error", metrics =["mse"])

#Fits model
history = model.fit(np.array(X_train), np.array(y_train), batch_size=2, epochs = Epochs, shuffle=True, validation_data = (X_test, y_test), verbose = 0)
...
plt.title('Model Train Loss Lidar-Sonar')
...
y_test_pred = model.predict(X[:1000]) #X_test
...
model.save("ANN_sonar_fusion4.h5")
print("Saved model to disk")

#GEnerating graphs.
fig, ax = plt.subplots(1, figsize=(8, 6))
fig.suptitle('ANN Testing Lidar-Sonar', fontsize=15)
...
fig.suptitle('ANN Training Lidar-Sonar ', fontsize=15)
plt.ylabel('Distance to obstacle [m]')
plt.xlabel('No. of measurement')
